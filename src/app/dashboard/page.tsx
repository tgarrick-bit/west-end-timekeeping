...
export default function EmployeeDashboard() {
  ...
  const checkUserRoleAndRedirect = async () => {
    try {
      ...
      console.log('Dashboard - Employee data:', employeeData);
      
      // If no employee record exists, check email pattern as fallback
      if (!employeeData) {
        console.log('No employee record found, checking email...');
        const userEmail = user.email?.toLowerCase() || '';
        
        if (userEmail.includes('admin')) {
          console.log('Email contains admin, redirecting to admin dashboard');
          router.push('/admin');
          return;
        } else if (userEmail.includes('manager') || userEmail === 'sarah.johnson@westend-test.com') {
          console.log('Email indicates manager, redirecting to manager dashboard');
          router.push('/manager');
          return;
        }
      }
      ...
      console.log('Employee role detected, staying on dashboard');
                        ...
                        <div className="flex items-center gap-3">
                          <div>
                            ...
                            <p className="text-xs text-gray-600 mt-1">
                              {timecard.total_hours} hrs • {formatCurrency(timecard.total_amount || 0)}
                            </p>
                          </div>
