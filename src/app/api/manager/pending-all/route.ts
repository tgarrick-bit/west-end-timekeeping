...
export async function GET(request: Request) {
  try {
    ...
    // Role check: only admins, managers, time_approvers
    ...
    if (!currentEmp || !['admin', 'manager', 'time_approver'].includes(currentEmp.role)) {
      return NextResponse.json({ error: 'Forbidden: manager or admin role required' }, { status: 403 });
    }

    // Get employees managed by this user
    const { data: myEmployees, error: empError } = await supabase
      .from('employees')
      .select('id')
    ...
    if (employeeIds.length === 0) {
      return NextResponse.json({
        items: [],
        stats: { pendingTimesheets: 0, pendingExpenses: 0, urgentItems: 0, totalPendingHours: 0, totalPendingAmount: 0 }
      });
    }

    // Fetch submitted timesheets for managed employees
    ...
    // Fetch submitted expense reports for managed employees
    ...
    if (expenses) {
      expenses.forEach(expense => {
        const totalAmount = expense.items?.reduce((sum: number, item: any) =>
          sum + (item.amount || 0), 0) || expense.total_amount || 0;
    ...
    const stats = {
      ...
      totalPendingAmount: expenses?.reduce((sum, e) => {
        const total = e.items?.reduce((s: number, i: any) => s + (i.amount || 0), 0) || e.total_amount || 0;
        return sum + total;
      }, 0) || 0
    };
