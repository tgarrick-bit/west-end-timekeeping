...
interface NotificationContextType {
  ...
  // Email integration
  sendEmailNotification: (toEmail: string, notification: Notification, customData?: Record<string, any>) => Promise<boolean>;
  ...
  // Bulk operations
  sendBulkNotifications: (userIds: string[], type: NotificationType, metadata?: Record<string, any>) => void;
...
export function NotificationProvider({ children }: NotificationProviderProps) {
  ...
  const crossCheckTimesheetExpenses = useCallback(async (
    employeeId: string, 
    period: string
  ): Promise<CrossCheckResult> => {
    return await notificationService.crossCheckTimesheetExpenses(employeeId, period);
  }, []);

  // Create cross-check notifications
  const createCrossCheckNotifications = useCallback((employeeId: string, period: string) => {
  ...
  const sendEmailNotification = useCallback(async (
    ...
    return await emailService.sendEmailNotification(toEmail, notification, customData);
  }, []);

  // Test email configuration
  const testEmailConfiguration = useCallback(async (): Promise<{ success: boolean; message: string }> => {
  ...
  const sendBulkNotifications = useCallback((userIds: string[], type: NotificationType, metadata?: Record<string, any>) => {
    userIds.forEach(userId => {
      notificationService.createNotification(type, userId, undefined, 'system', metadata);
    });
  }, []);
  ...
  const clearAllNotifications = useCallback((userId: string) => {
    ...
    // For now, we'll just clear from local storage
    try {
