...
// Simple CSS approach - no Tailwind complications!
...
export default function SupervisorPendingView() {
  ...
  const fetchPendingItems = async () => {
    try {
      const response = await fetch('/api/supervisor/pending-all');
  ...
  const approveSelected = async () => {
    ...
    try {
      const response = await fetch('/api/supervisor/approve-bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          itemIds: selectedItemsData,
          approvalType: 'both',
      ...
      if (result.success) {
        showMessage('success', `Successfully approved ${result.approvedCount} items`);
        setSelectedItems(new Set());
        fetchPendingItems();
      } else {
      ...
      showMessage('error', 'Failed to approve items');
      console.error(error);
    }
  };
  ...
  return (
    <div className={styles.container}>
      ...
      {/* Header Summary Card - THE MOST IMPORTANT PART */}
